...
const ROUTES: RouteItem[] = [
  { key: "zeppelin", Icon: Building2, photo: PHOTOS.friedrichshafen },
  { key: "lindau", Icon: Anchor, photo: PHOTOS.lindau },
  { key: "konstanz", Icon: MapPin, photo: PHOTOS.konstanz },
  { key: "alps", Icon: Mountain, photo: PHOTOS.alps },
  { key: "islands", Icon: Compass, photo: PHOTOS.mainau },
  { key: "individuell", Icon: Sparkles },
];
      ...
      <div className="container-x">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {ROUTES.map(({ key, Icon, photo }, index) => {
            const style: CSSProperties = { transitionDelay: `${index * 60}ms` };
            ...
            return (
              <button
                ...
                style={style}
                className="reveal group relative h-72 cursor-pointer rounded-2xl text-left text-white shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-brand-500/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-950"
              >
                <span
                  aria-hidden
                ...
                >
                  {photo ? (
                    ...
                    <span className="absolute inset-0 block bg-gradient-to-br from-brand-700 via-brand-500 to-brand-300 transition-transform duration-700 ease-out group-hover:scale-110 dark:from-brand-800 dark:via-brand-600 dark:to-brand-400" />
                  )}
                  <span className="absolute inset-0 bg-gradient-to-t from-slate-950/85 via-slate-950/40 to-slate-950/10" />
                </span>
                ...
                <span className="relative flex h-full flex-col justify-between p-6">
                  <span className="flex items-start justify-between gap-3">
                    ...
                    />
                    <span
                      className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1.5 text-xs font-semibold text-white shadow-sm ring-1 ring-white/30 backdrop-blur-md"
                      title={durationLabel}
                    >
                  ...
                  <span className="block">
                    ...
                    <span className="mt-2 line-clamp-3 block text-sm leading-relaxed text-white/90 drop-shadow-sm md:text-[0.95rem]">
                      {body}
                    </span>
